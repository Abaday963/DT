...
public class ArrowAmmunition1 : MonoBehaviour, IAmmunition
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (ammunitionComponent != null)
        {
            ...
            return; // Добавлено return, чтобы выйти из метода без вызова OnImpact()
        }

        // Вызываем метод интерфейса IAmmunition только если это не амуниция
        OnImpact();
    ...
    {
        // Проверяем, не столкнулись ли мы с другим боеприпасом
        Collider2D[] colliders = Physics2D.OverlapPointAll(transform.position);
        bool hitAmmo = false;

        Debug.Log("OnImpact: Найдено " + colliders.Length + " коллайдеров в точке столкновения");

        foreach (Collider2D col in colliders)
        {
            if (col.gameObject != gameObject)
            {
                Debug.Log("Проверяем объект: " + col.gameObject.name);

                if (col.gameObject.GetComponent<IAmmunition>() != null)
                {
                    Debug.Log("Обнаружена амуниция: " + col.gameObject.name + ", пропускаем обработку столкновения");
                    hitAmmo = true;
                    break;
                }
            }
        ...
        // Если столкнулись с амуницией, просто игнорируем это столкновение
        if (hitAmmo)
        {
            Debug.Log("Столкновение с амуницией, выходим без обработки");
            return;
        }

        Debug.Log("Останавливаем движение стрелы и меняем тип на Kinematic");
        // Остановка движения стрелы только если это не амуниция
        ...
        {
            Debug.Log("Пытаемся прикрепить стрелу к объекту");
            foreach (Collider2D col in colliders)
            {
                if (col.gameObject != gameObject && col.gameObject.GetComponent<IAmmunition>() == null)
                {
                    Debug.Log("Прикрепляем стрелу к объекту: " + col.transform.name);
